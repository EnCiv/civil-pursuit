...
!(function () {
  ...
  function indexCss(done) {
    async.series([lessToCss.bind(null, paths.less + '/synapp.less', paths.css + '/index.css'), minifyCss.bind(null, paths.css + '/index.css', paths.css + '/index.min.css')], done);
  }
  ...
  function browserifyPages(pages, done) {
    ...
    fs.readdir(paths.pages, function (error, files) {
      ...
      async.each(files, function (page, done) {
        ...
        var dest = fs.createWriteStream(path.join(paths.js, 'page-' + S(page).humanize().slugify().s + '.js'));
    ...
    fs.readdir(paths.pages, function (error, files) {
      ...
      async.each(files, function (page, done) {
        ...
        fs.stat(ctrl, function (error, stat) {
          ...
          cp.exec(util.format('%s %s -o %s', path.join(paths.node_bin, 'uglifyjs'), ctrl, path.join(paths.js, 'page-' + S(page).humanize().slugify().s + '.min.js')), done);
        });
      }, done);
    });
  }
  ...
  args = args.map(function (arg) {
    if (arg.name === 'browserifyPages') {
      var browserifyPagesWrapper = function (done) {
        this.fn(this.pages, done);
      };

      var pages = process.argv.filter(function (arg, i) {
        return i > 2;
      });
