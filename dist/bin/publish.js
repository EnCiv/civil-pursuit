...
!(function () {
  ...
  var tests = [{ name: 'app/bin/build.js css' }, { name: 'app/bin/build.js browserify-pages' }, { name: 'app/bin/build.js uglify-pages' }, { name: 'git commit -am "' + deployMessage.replace(/\s/g, '%%%%%') + '"', ok: [0, 1, 8792, 7182] },
  ...
  require('async').eachSeries(tests, function (test, cb) {
    var action = test;
    ...
    cp.spawn(cmd, chunks, { stdio: 'inherit' }).on('exit', function (status) {
