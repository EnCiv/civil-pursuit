...
!(function () {
  ...
  function render() {
    ...
    */
    ...
    new Upload(null, this.find('upload button'), this.template.find('.user-image-container'), function (error, file) {
    ...
    var citizenships = [$(this.find('citizenship')[0]).val(), $(this.find('citizenship')[1]).val()];
    ...
    this.find('citizenship').each(function (index) {
      ...
      select.on('change', function () {
        if (select.val()) {
          app.socket.on('citizenship set', function () {
    ...
    this.find('dob').on('change', function () {
      ...
      app.socket.on('birthdate set', function () {
        console.log('birthdate set');
      }).emit('set birthdate', app.socket.synuser, $(this).val());
    });
    ...
    this.find('gender').on('change', function () {
      ...
      if ($(this).val()) {
        app.socket.on('gender set', function () {
          console.log('gender set');
        }).emit('set gender', app.socket.synuser, $(this).val());
      }
    });
  }
