...
!(function () {
  ...
  function isItem(id, client, done) {
    ...
    domain.run(function () {
      ...
      Item.findById(id).exec().then(function (item) {
      ...
      function checkYourself(error, item) {
        ...
        })).getAttribute('#item-' + id + '>.item-media-wrapper>.item-media img', 'src', domain.intercept(function (src) {
        ...
        .getText('#item-' + id + '>.item-text .item-subject', domain.intercept(function (text) {
        ...
        .isVisible('#item-' + id + '>.item-buttons>button.item-toggle-promote>.promoted', domain.intercept(function (isVisible) {
          console.log('\t  ✓'.green, ('item#' + item.id + ' > Promotion Counter is visible').grey);
        })).getText('#item-' + id + '>.item-buttons>button.item-toggle-promote>.promoted', domain.intercept(function (text) {
        ...
        .isVisible('#item-' + id + '>.item-buttons>button.item-toggle-details>.promoted-percent', domain.intercept(function (isVisible) {
          console.log('\t  ✓'.green, ('item#' + item.id + ' > Popularity is visible').grey);
        })).getText('#item-' + id + '>.item-buttons>button.item-toggle-details>.promoted-percent', domain.intercept(function (text) {
        ...
        .isVisible('#item-' + id + '>.item-buttons > div > .related > .children-count', domain.intercept(function (isVisible) {
          console.log('\t  ✓'.green, ('item#' + item.id + ' > Children count is visible').grey);
        })).getText('#item-' + id + '>.item-buttons  > div > .related > .children-count', domain.intercept(function (text) {
          parseInt(text).should.be.a.Number.which.is.above(-1);
