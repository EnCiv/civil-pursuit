...
!(function () {
  ...
  var deps = ['path', 'mongoose', 'bcrypt', 'cloudinary', 'syn/config.json', 'syn/models/User/schema', 'syn/models/User/pre/save', 'syn/lib/util/to-slug', 'syn/lib/util/to-camel-case'];
  ...
  function run(path, mongoose, bcrypt, cloudinary, config, schema, preSave, toSlug, toCamelCase) {
    ...
    statics.forEach(function (_static) {
      UserSchema.statics[toCamelCase(_static)] = require('syn/models/User/statics/' + toSlug(_static));
    });
    ...
    virtuals.forEach(function (virtual) {
      ...
      UserSchema.virtual(toCamelCase(virtual)).get(require(_virtual));
    });
