...
async function getStatementIds(discussionId, round, userId) {
  ...
  console.log('Before processing ShownGroups or ShownStatements:', JSON.stringify(dis, null, 2))

  if (dis.ShownGroups[round]?.at(-1)?.shownCount < Math.pow(dis.group_size, round + 1)) {
     console.log(
       'shownCount condition:',
       dis.ShownGroups[round].at(-1).shownCount,
       'required:',
       Math.pow(dis.group_size, round + 1)
     )
  ...
  console.log('Returning statementIds:', statementIds)
  return statementIds
}
