...
function test(props) {
  ...
  return testWrapper(
    'Story -> Evaluation -> Save Right Feedback on Finish',
    ...
    wrappers => it => {
      ...
      it('should fill feedback', () => wrappers.driver.client.setValue(
        `${selectors.evaluation.id.prefix}${locals.items[0]._id} [data-screen="phone-and-up"] .promote-right ${selectors.evaluation.feedback}`, locals.candidate
      ));
      ...
      it('Verify feedback', it => {
        it('should get item from DB', () =>
          Feedback.find({ item : locals.items[0] })
