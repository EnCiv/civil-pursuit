    ...
    wrappers => it => {
      ...
      it('should see item 5', () =>
        wrappers.driver.isVisible(
          `${selectors.item.id.prefix}${locals.items[4]._id}`, 2500
        )
      );

      it('should click on evaluation toggler', () =>
        wrappers.driver.client.click([
          `${selectors.item.id.prefix}${locals.items[4]._id}`,
      ...
      it('Data items', it => {
        it('should get data items', () => wrappers.driver.client.getAttribute(
          selectors.evaluation.id.prefix + locals.items[4]._id,
          'data-items'
        ).then(items => { locals.dataItems = items }));

        it('should have 5 items', () => {
          locals.dataItems.split(',').should.have.length(5)
        });
      });
      ...
      it('3rd cycle', it => {
        ...
        it('Left item', it => {
          ...
          it('should be the evaluee', () => locals.leftId.should.be.exactly(locals.items[4]._id.toString()));
        });
        ...
        it('Shows details view', describe.use(() => isDetailsView(
          wrappers.driver, locals.items[4]
        )));
      });
    }
  );
