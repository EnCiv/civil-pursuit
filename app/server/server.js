...
class HttpServer extends EventEmitter {
  ...
  signers() {
    this.app.post('/sign/in', signInRoute, this.setUserCookie, function (req, res) {
      res.send({
        in: true,
        id: req.user._id,
      })
    ...
    this.app.all('/sign/up', signUpRoute, this.setUserCookie, function (req, res) {
      res.json({
        up: true,
        id: req.user._id,
      })
    ...
    this.app.all('/tempid', tempIdRoute, this.setUserCookie, function (req, res) {
      res.json({
        up: true,
        id: req.user._id,
      })
  ...
  getLandingPage() {
    try {
      if (process.env.LANDING_PAGE_REDIRECT_PATH) {
        this.app.get('/', (req, res, next) => {
          res.redirect(process.env.LANDING_PAGE_REDIRECT_PATH)
        })
      } else {
        this.app.get('/', this.setUserCookie, serverReactRender.bind(this))
      }
    } catch (error) {
      this.emit('error', error)
    }
  ...
  getItemPage() {
    this.app.get(
      ...
      (req, res, next) => {
        ...
        if (!shortId) next()
        try {
          Item.findOne({ id: shortId }).then(item => {
            if (!item) {
              return next()
            }
            item.toPanelItem(userId).then(item => {
              req.panels = {}
              if (item & item.parent) {
    ...
    try {
      this.app.get(
        ...
        (req, res, next) => {
          ...
          try {
            Type.findOne({ id: 'Polzc' }).then(type => {
              if (!type) {
              ...
              Item.findOne({ id: 'bvuDs' }).then(item => {
                if (!item) {
                  return next()
                }
                ...
                Item.getPanelItems(query, userId).then(results => {
  ...
  getPanelPage() {
    this.app.get(
      '/items/:panelShortId/:panelParent?',
      (req, res, next) => {
        var userId = req.cookies.synuser && req.cookies.synuser.id ? req.cookies.synuser.id : null
        try {
          Type.findOne({ id: req.params.panelShortId }).then(type => {
            if (!type) {
              return next(new Error('No such type'))
            }
            ...
            Item.findOne({ id: req.params.panelParent }).then(item => {
              if (!item) {
                return next()
              }
