...
export class RASPRuleList extends ReactActionStatePathClient {
    ...
    actionToState(action,rasp) {
        ...
        if(action.type==="TOGGLE_QBUTTON") {
            ...
            delta.creator=false;
            this.qaction(()=>this.props.rasp.toParent({type: "DECENDANT_FOCUS"})); // forces an updateHistory
        } else if (action.type==="TOGGLE_CREATOR"){
            delta.creator= !rasp.creator;
            this.qaction(()=>this.props.rasp.toParent({type: delta.creator? "DECENDANT_FOCUS" : "DECENDANT_UNFOCUS"})); // forces an updateHistory
    ...
    componentWillReceiveProps(newProps){
        if(!newProps.rasp.itemId!=='redirect' && this.state.typeList.length && ((typeof newProps.sections['unsorted'] === 'undefined') || newProps.sections['unsorted'].length===0) && newProps.sections['agree'] && newProps.sections['agree'].length && !this.neededInputAtStart)
