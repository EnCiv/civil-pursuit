  ...
  render() {
    ...
    if (this.state.ready) {
      ...
      if (
        buttons &&
        buttons.zip &&
        buttons.zip.some(zip => zip == (userInfo.street_address && userInfo.street_address.zip)) &&
        userInfo.street_address.validatedAt
      ) {
        // we have a match
        const newPanel = {
          parent: panel.parent,
        ...
        return <TypeComponent {...this.props} component={this.state.typeList[0].component} panel={newPanel} />
      } else {
        //no match
        setTimeout(() => {
        ...
        let instruction = (
          <div className="instruction-text">
            Thanks for signing up! You will be notified when there is a deliberative survey taking place in your area.
          </div>
        )

        if (panel.type && panel.type.instruction) {
          instruction = <Instruction>{panel.type.instruction}</Instruction>
        }

        return (
          <Panel ref="panel" heading={[<h4>{title}</h4>]}>
            {instruction}
          </Panel>
        )
      }
