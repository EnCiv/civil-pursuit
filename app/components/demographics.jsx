...
import StartingBlocRace from './prof-components/starting-bloc-race'
  ...
  render() {
    ...
    let racesList = races.map(race => (
      <Row key={race._id} className="demographics-race" id={`demographics-race-${race._id}`}>
        <Column className="gutter">{race.name}</Column>
    ...
    let startingBlocRace = user.starting_bloc_race ? (
      <Row>
        <Column className="gutter">Race (form Starting Bloc community application from):</Column>
        <Column>
    ...
    let education = educations.map(educ => (
      <option value={educ._id} key={educ._id}>
        {educ.name}
      </option>
    ))
    ...
    let employmentsList = employments.map(employment => (
      <option value={employment._id} key={employment._id}>
        {employment.name}
      </option>
    ))
    ...
    return (
      <section className="demographics">
        <section style={{ width: '50%', float: 'left' }}>
          <Image
            src="https://res.cloudinary.com/hscbexf6a/image/upload/v1423261951/y1qxy2fwmgiike5gx7ey.png"
            responsive
          />
        </section>
        ...
        <section className="gutter">
          <h2>Demographics</h2>
          <p>
            We use this information to make sure that we have balanced participation. When we see too little
            participation in certain demographics then we increase our efforts to get more participation there
          </p>
        </section>

        <Row>
          <Column className="gutter">Race:</Column>
          <Column>{racesList}</Column>
        </Row>
        ...
        <section className="gutter">
          <Row baseline className="gutter-y">
            <Column span="25">Education</Column>
            <Column span="75">
              <Select
                block
                medium
          ...
          <Row baseline className="gutter-y">
            <Column span="25">Relationship</Column>
            <Column span="75">
              <Select
                block
                medium
                ...
                onChange={this.setRelationship.bind(this)}
              >
                <option value="">Choose one</option>
                {relationships}
              </Select>
          ...
          <Row baseline className="gutter-y">
            <Column span="25">Employment</Column>
            <Column span="75">
              <Select
                block
                medium
                ...
                onChange={this.setEmployment.bind(this)}
              >
                <option value="">Choose one</option>
                {employmentsList}
              </Select>
