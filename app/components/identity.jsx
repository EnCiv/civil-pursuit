...
import Gender from './prof-components/gender'
import Birthdate from './prof-components/birthdate'
import GenderIdentity from './prof-components/gender-identity'
...
class Identity extends React.Component {
  ...
  saveCitizenship(e) {
    ...
    setUserInfo({ citizenship: citizenship }, user => {
      this.setState({ user })
    })
  }
  ...
  saveDualCitizenship(e) {
    ...
    setUserInfo({ dualcitizenship: dualCitizenship }, user => {
      this.setState({ user })
    })
  }
  ...
  render() {
    ...
    return (
      <section className={selectors.identity.selector.replace(/\./g, ' ')}>
        <Row>
          <Column span="50" className={`gutter ${selectors.identity.uploader.replace(/\./g, ' ')}`}>
        ...
        <section className="gutter">
          ...
          <Row baseline className="gutter-y">
            <Column span="25">Citizenship</Column>
            <Column span="75">
              <Select
                block
                medium
                onChange={this.saveCitizenship.bind(this)}
                ref="citizenship"
          ...
          <Row baseline className="gutter-y" style={citizenship ? {} : { display: 'none' }}>
            <Column span="25">Second Citizenship?</Column>
            <Column span="75">
              <Select
                block
                medium
                onChange={this.saveDualCitizenship.bind(this)}
                ref="dualCitizenship"
                defaultValue={dualCitizenship}
              >
