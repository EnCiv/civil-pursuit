...
class PanelItems extends React.Component {
  ...
  componentWillReceiveProps(newProps){
    if(!this.history && newProps.panel && newProps.panel.type){
      let typeSId=newProps.panel.type.id || 'notypeid';
      let parentSId= newProps.panel.parent ? newProps.panel.parent.id || 'noparentid' : 'noparent';
      window.history.pushState({},"",window.location.pathname+'items/'+typeSId+'/'+parentSId);
      this.history=true;
    }
  }
  ...
  toggle (itemId, section) {
    ...
    if (section == 'creator') {
      return this.setState({ active : { item : itemId, section: this.state.active.section === section ? null : section }});
    }
 ...
 //       }
    ...
    return (
      <section id               =     "syn-panel-items">
        <Panel
          noHeading={this.state.active.item && this.state.active.section ==='subtype' && type.visualMethod && type.visualMethod ==="ooview"}
          vs = {this.state.vs}
          className   =   { name }
          ref         =   "panel"
          heading     =   {[
