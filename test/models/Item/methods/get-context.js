! function () {
  ...
  function Item_getContext (done) {
    ...
    var _transformers   =     'syn/models/transformers';

    var transformers    =     {
      includeMethods    :     require(_transformers + '/include-methods')
    };
    ...
    Test.suite('Item Model :: Get Context', {
      ...
      'Should not emit error': function (done) {
        ...
        Item.findOne(function (error, _item) {
          ...
          item.getContext(function (error, context) {
            if ( error ) {
              throw error;
            }

            item = item.toObject({
              transform       :     function (ret, doc, options) {
                transformers['include methods'](ret, doc, options);
              }
            });
